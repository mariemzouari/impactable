...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Use session ID if available, otherwise check POST (fallback)
    $userId = ($currentUserId > 0) ? $currentUserId : (isset($_POST['userId']) ? intval($_POST['userId']) : 0);

    // Validation
    if (empty($message) || strlen($message) < 10) {
        $error = 'Le message doit contenir au moins 10 caractères';
    } elseif ($userId <= 0) {
        $error = 'ID utilisateur invalide (Non connecté)';
    } else {
        try {
                ...
                <form method="POST" action="" id="reponseForm" onsubmit="return validateAjouterReponseForm(event)">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="message" name="message"
                            placeholder="Écrivez votre réponse ici..."><?= isset($_POST['message']) ? htmlspecialchars($_POST['message']) : '' ?></textarea>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="userId" name="userId"
                            value="<?= ($currentUserId > 0) ? $currentUserId : (isset($_POST['userId']) ? htmlspecialchars($_POST['userId']) : '') ?>"
                            <?= ($currentUserId > 0) ? 'readonly style="background-color: #f0f0f0; cursor: not-allowed;"' : 'placeholder="Entrez l\'ID de l\'utilisateur"' ?>>
                        <?php if ($currentUserId > 0): ?>
                            <small style="color: var(--moss);"><i class="fas fa-user-check"></i> Connecté en tant qu'utilisateur
                                #<?= $currentUserId ?></small>
                        <?php endif; ?>
                    </div>
