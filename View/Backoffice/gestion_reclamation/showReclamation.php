    ...
    <div class="container">
        ...
        <?php if ($reclamation): ?>
            <div class="info-section">
                ...
                <p><strong>Date de création:</strong> <?= date('d/m/Y à H:i', strtotime($reclamation['dateCreation'])) ?>
                </p>
                <p><strong>Dernière modification:</strong>
                    <?= date('d/m/Y à H:i', strtotime($reclamation['derniereModification'])) ?></p>
            ...
            <?php if ($emotionAnalysis): ?>
                <div class="info-section"
                    style="background: <?= $emotionAnalysis['couleur_bg'] ?>; border-left: 5px solid <?= $emotionAnalysis['couleur'] ?>;">
                    ...
                    <div style="display: flex; align-items: center; gap: 20px; margin: 15px 0; flex-wrap: wrap;">
                        ...
                        <div>
                            ...
                            <div style="color: #5E6D3B;">Intensité : <?= $emotionAnalysis['intensite'] ?> (Score:
                                <?= $emotionAnalysis['score'] ?>)</div>
                        </div>
                        <div style="flex: 1; min-width: 200px;">
                            <div
                                style="background: rgba(255,255,255,0.5); border-radius: 10px; height: 15px; overflow: hidden;">
                                <div
                                    style="height: 100%; width: <?= $emotionAnalysis['intensite_pourcent'] ?>%; background: <?= $emotionAnalysis['couleur'] ?>; border-radius: 10px;">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <?php if (!empty($emotionAnalysis['mots_detectes'])): ?>
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 8px; margin: 10px 0;">
                            <?php foreach ($emotionAnalysis['mots_detectes'] as $mot): ?>
                                <span
                                    style="background: white; color: <?= $emotionAnalysis['couleur'] ?>; padding: 5px 12px; border-radius: 20px; font-size: 0.9em; font-weight: 500;">
                    ...
                    <div style="background: white; padding: 15px; border-radius: 10px; margin-top: 15px;">
                        <p style="margin: 0 0 10px 0;"><strong><?= $emotionAnalysis['conseil_agent']['icon'] ?>
                                <?= $emotionAnalysis['conseil_agent']['titre'] ?></strong></p>
                        <p style="margin: 5px 0; color: #5E6D3B;"><i class="fas fa-lightbulb"></i>
                            <?= $emotionAnalysis['conseil_agent']['conseil'] ?></p>
                        <?php if ($emotionAnalysis['conseil_agent']['a_eviter'] !== '-'): ?>
                            <p style="margin: 5px 0; color: #B47B47; font-size: 0.9em;"><i class="fas fa-exclamation-triangle"></i>
                                À éviter : <?= $emotionAnalysis['conseil_agent']['a_eviter'] ?></p>
                        <?php endif; ?>
                    </div>

                    <div
                        style="background: linear-gradient(135deg, #5E6D3B, #4B2E16); color: white; padding: 15px; border-radius: 10px; margin-top: 15px;">
            ...
            <div class="action-buttons">
                <a href="../reponsecrud/ajouter_reponse.php?reclamation_id=<?= $reclamation['id'] ?>"
