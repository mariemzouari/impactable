...
if (
    ...
    if (
        ...
        // Use session ID if available, otherwise check POST (for fallback/admin manual entry if needed, though request implies strict session usage)
        // Adjusting to strictly use session ID if logged in, or generic fallback if not (though dashboard implies login)
        $idUtilisateur = $userId ? $userId : (isset($_POST["utilisateurId"]) ? intval($_POST["utilisateurId"]) : 0);
        ...
        <form action="" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" name="utilisateurId" id="utilisateurId"
                    value="<?= $userId ? htmlspecialchars($userId) : '' ?>" <?= $userId ? 'readonly style="background-color: #f0f0f0; cursor: not-allowed;"' : 'placeholder="Entrez l\'ID de l\'utilisateur"' ?>>
                <?php if ($userId): ?>
