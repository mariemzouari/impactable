...
            <form action="index.php?page=admin-events-handle-modify" method="POST" novalidate enctype="multipart/form-data">
              <input type="hidden" name="id" value="<?php echo htmlspecialchars($eventId); ?>">
              
              <div class="form-group"><label for="titre">Titre</label><input type="text" id="titre" name="titre" class="input" value="<?php echo htmlspecialchars($evenement['titre']); ?>"></div>
              
              <div class="form-group"><label for="date_event">Date & Heure</label><input type="text" id="date_event" name="date_event" class="input" value="<?php echo htmlspecialchars($evenement['date_event']); ?>"></div>
...
              <div class="form-group"><label for="description">Description</label><textarea id="description" name="description" class="input" rows="5"><?php echo htmlspecialchars($evenement['description']); ?></textarea></div>
              
              <div class="form-group"><label for="capacite_max">Nombre maximum de participants</label><input type="number" id="capacite_max" name="capacite_max" class="input" value="<?php echo htmlspecialchars($evenement['capacite_max'] ?? '0'); ?>"></div>

              <div class="form-group"><label for="location">Lieu de l'événement</label><input type="text" id="location" name="location" class="input" value="<?php echo htmlspecialchars($evenement['location'] ?? ''); ?>"></div>
              
              <div class="form-group">
                  <label for="image">Image de l'événement (laisser vide pour ne pas changer)</label>
                  <input type="file" id="image" name="image" class="input">
                  <?php if (!empty($evenement['image'])): ?>
                      <div style="margin-top: 10px;">
                          <p>Image actuelle :</p>
                          <img src="../../assets/uploads/<?= htmlspecialchars($evenement['image']) ?>" alt="Image de l'événement" style="max-width: 200px; border-radius: 6px;">
                      </div>
                  <?php endif; ?>
              </div>
