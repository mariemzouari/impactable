...
<div class="container">
    ...
    <section class="section">
        <div style="max-width:1200px; margin:0 auto; padding:3rem 0;">
            ...
            <div style="margin-bottom: 2rem; text-align: center; display: flex; justify-content: center; gap: 10px;">
                <form action="events-list.php" method="GET" style="display: flex; gap: 10px;">
                    <input type="text" name="search" placeholder="Rechercher un événement..." style="padding: 10px; width: 300px; border-radius: 5px; border: 1px solid #ccc;" value="<?= htmlspecialchars($_GET['search'] ?? '') ?>">
                    <button type="submit" style="padding: 10px 20px; border-radius: 5px; border: none; background-color: #5e6d3b; color: white; cursor: pointer;">Rechercher</button>
                    <select name="sort" onchange="this.form.submit()" style="padding: 10px; border-radius: 5px; border: 1px solid #ccc;">
                    ...
                    <?php foreach($events as $event): ?>
                        <div style="background:#fffaf5; border-radius:12px; padding:1.5rem; box-shadow:0 4px 12px rgba(75,46,22,0.08); transition:transform 0.2s, box-shadow 0.2s; border-left: 4px solid #5e6d3b;" onmouseover="this.style.transform='translateY(-4px)'; this.style.boxShadow='0 6px 16px rgba(75,46,22,0.12)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 12px rgba(75,46,22,0.08)'">
                            ...
                            <p style="margin:0.5rem 0; color:#6b4b44; font-size:0.9rem;">
                                ...
                                ?>
                                <?= $metierIcon ?>
                                <img src="<?= $logoPath ?>" alt="<?= $category ?> logo" style="height:16px; width:16px; margin-right:0.5rem; object-fit: contain;">
                            ...
                            <?php if (!empty($event['location'])): ?>
                                <div style="margin:1rem 0;">
                                    <h4 style="margin:0 0 0.5rem 0; color:#4b2e16; font-weight:600; font-size:1rem;">
                                        <i class="fas fa-map-marker-alt" style="color:#b47b47; margin-right:0.5rem;"></i> Lieu de l'événement:
                                    </h4>
                                    <a href="https://www.google.com/maps/search/?api=1&query=<?= urlencode($event['location']) ?>" target="_blank" class="btn-map">
                            ...
                            ?>
                            <p style="margin:0.5rem 0; color:#6b4b44; font-size:0.9rem;">
                                <i class="fas fa-users" style="color:#5e6d3b; margin-right:0.5rem;"></i> Inscrits: <?= $participantsCount ?>
                            </p>
                            <p style="margin:0.5rem 0; color:#6b4b44; font-size:0.9rem;">
                                <i class="fas fa-chair" style="color:#b47b47; margin-right:0.5rem;"></i> Places restantes: <?= max(0, $placesRestantes) ?> / <?= htmlspecialchars($event['capacite_max']) ?>
                            </p>
                            <div style="display:flex; gap:10px; margin-top:1rem;">
                                ...
                                <div style="flex:1;">
                                    <?php if ($placesRestantes > 0): ?>
                                        <button onclick="openDetailedParticipationModal(<?= intval($event['id']) ?>, <?= $userId !== null ? $userId : 'null' ?>)" style="width:100%; display:flex; align-items:center; justify-content:center; padding:10px 16px; background:#b47b47; color:white; border-radius:8px; text-decoration:none; font-weight:600; transition:all 0.2s; border:none; cursor:pointer; font-size:inherit; font-family:inherit;" onmouseover="this.style.background='#8f5e2f'" onmouseout="this.style.background='#b47b47'">
                                            <i class="fas fa-heart" style="margin-right:0.5rem;"></i> Participer
                                        </button>
                                    <?php else: ?>
                                        <button disabled style="width:100%; padding:10px 16px; background:#ccc; color:#666; border-radius:8px; border:none; cursor:not-allowed; font-weight:600;">
                                            Complet
                                        </button>
                                    <?php endif; ?>
                                    <div class="success-msg" style="display: none; color: #27ae60; text-align: center; font-weight: 600; font-size:0.9rem; margin-top:8px;">✔ Inscription enregistrée !</div>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
