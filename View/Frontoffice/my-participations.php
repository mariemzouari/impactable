...
// Authentication check - Redirection REMOVED AS PER USER REQUEST
if ($userId) {
    ...
    <div class="container">
        ...
        <header class="site-header" role="banner">
            ...
            <div class="header-actions">
                <?php if ($user): ?>
                    <span class="user-welcome">Bonjour, <?= htmlspecialchars($user['prenom']); ?></span>
            ...
            <div class="panel-footer">
                <div class="user-profile">
                    <div class="user-avatar">
                        <?= $user ? strtoupper(substr($user['prenom'], 0, 1) . substr($user['nom'], 0, 1)) : 'VS' ?>
                    </div>
                    <div class="user-info">
        ...
        <main style="max-width: 1100px; margin: 40px auto; padding: 0 20px;">
            <div style="margin-bottom: 40px;">
                <h1 style="font-size: 2.2rem; color: #4b2e16; margin-bottom: 10px;"><i class="fas fa-heart"
                        style="color:#ffb3b3;"></i> Mes Participations</h1>
                <p style="color: #6b4b44; font-size: 1.05rem;">Consultez et gérez toutes vos inscriptions aux événements
                </p>
            </div>
            ...
            <div
                ...
                <div
                    style="background: #fdfafa; padding: 20px; border-radius: 12px; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                    <div style="font-size: 1.1rem; color: #4b2e16; font-weight: 500; margin-bottom: 8px;">Total
                        Événements</div>
                ...
                <div
                    ...
                    <div style="font-size: 1.1rem; color: #4b2e16; font-weight: 500; margin-bottom: 8px;">Total
                        Participations</div>
                    <div style="font-size: 2.2rem; font-weight: 700; color: #b47b47;"><?= $totalUserParticipations ?>
                    </div>
                </div>
                <div
                    style="background: #fdfafa; padding: 20px; border-radius: 12px; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                    <div style="font-size: 1.1rem; color: #4b2e16; font-weight: 500; margin-bottom: 8px;">Événements à
                        venir</div>
                    <div style="font-size: 2.2rem; font-weight: 700; color: #27ae60;"><?= $userUpcomingParticipations ?>
                    </div>
                </div>
                <div
                    style="background: #fdfafa; padding: 20px; border-radius: 12px; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                    <div style="font-size: 1.1rem; color: #4b2e16; font-weight: 500; margin-bottom: 8px;">Participation
                        Moyenne</div>
                    <div style="font-size: 2.2rem; font-weight: 700; color: #f39c12;">
                        <?= htmlspecialchars($averageParticipation) ?>
                    </div>
                </div>
                <div
                    style="background: #fdfafa; padding: 20px; border-radius: 12px; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                    <div style="font-size: 1.1rem; color: #4b2e16; font-weight: 500; margin-bottom: 8px;">Événements
                        sans Participants</div>
                    <div style="font-size: 2.2rem; font-weight: 700; color: #e74c3c;"><?= $eventsWithoutParticipants ?>
                    </div>
                </div>
            </div>
            ...
            <div id="participationsList" style="display: grid; gap: 20px;">
                <?php if (empty($userParticipations)): ?>
                    <p style="text-align: center; color: #6b4b44;">Vous n'avez pas encore d'inscriptions à des événements.
                    </p>
                <?php else: ?>
                    <?php foreach ($userParticipations as $participation): ?>
                        ...
                        ?>
                        <div
                            style="background: #fffaf5; border-left: 4px solid <?= $borderColor ?>; padding: 20px; border-radius: 12px; box-shadow: 0 4px 12px rgba(75,46,22,0.08);">
                            <div style="display: flex; justify-content: space-between; margin-bottom: 15px;">
                                <div>
                                    <h3 style="font-size: 1.3rem; color: #4b2e16; font-weight: 600; margin: 0;">
                                        <?= htmlspecialchars($participation['nom_evenement']) ?>
                                    </h3>
                                    <div style="color: #6b4b44; font-size: 0.95rem;"><i class="fas fa-calendar"></i>
                                        <?= htmlspecialchars(date('d M Y, H', strtotime($participation['date_event']))) ?>h
                                    </div>
                                    <div style="color: #6b4b44; font-size: 0.95rem;"><i class="fas fa-user"></i> Participant:
                                        <?= htmlspecialchars($participation['prenom'] . ' ' . $participation['nom']) ?>
                                    </div>
                                    <div style="color: #6b4b44; font-size: 0.95rem;"><i class="fas fa-envelope"></i> Email:
                                        <?= htmlspecialchars($participation['email']) ?>
                                    </div>
                                </div>
                                <span class="<?= $statusClass ?>"
                                    style="padding: 6px 12px; border-radius: 20px; font-size: 0.85rem; font-weight: 600; background: <?= $bgColor ?>; color: <?= $textColor ?>;">
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px;">
                                <div style="display: flex; align-items: center; gap: 10px;">
                                    <div
                                        style="width: 32px; height: 32px; border-radius: 50%; background: #e1e8c9; display: flex; align-items: center; justify-content: center; color: #5e6d3b;">
                                        <i class="fas fa-map-marker-alt"></i>
                                    </div>
                                    <div>
                                        <div style="font-weight: 600; color: #4b2e16;">Lieu</div>
                                        <div style="color: #6b4b44; font-size: 0.9rem;">
                                            <?= htmlspecialchars($participation['adresse'] ?? 'Non spécifié') ?>
                                        </div>
                                    </div>
                                </div>
                                <div style="display: flex; align-items: center; gap: 10px;">
                                    ...
                                    <div>
                                        <div style="font-weight: 600; color: #4b2e16;">Accompagnants</div>
                                        <div style="color: #6b4b44; font-size: 0.9rem;">
                                            <?= htmlspecialchars($participation['nombre_accompagnants']) ?>
                                        </div>
                                    </div>
                                </div>
                                <div style="display: flex; align-items: center; gap: 10px;">
                                    ...
                                    <div>
                                        <div style="font-weight: 600; color: #4b2e16;">Besoins</div>
                                        <div style="color: #6b4b44; font-size: 0.9rem;">
                                            <?= htmlspecialchars($participation['besoins_accessibilite']) ?>
                                        </div>
                                    </div>
                                </div>
                                <div style="display: flex; align-items: center; gap: 10px;">
                                    ...
                                    <div>
                                        <div style="font-weight: 600; color: #4b2e16;">Inscrit le</div>
                                        <div style="color: #6b4b44; font-size: 0.9rem;">
                                            <?= htmlspecialchars(date('d M Y', strtotime($participation['date_inscription']))) ?>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div style="display: flex; gap: 10px; padding-top: 15px; border-top: 1px solid rgba(75,46,22,0.1);">
                                <a href="event/event-detail.php?id=<?= htmlspecialchars($participation['id_evenement']) ?>"
                                    style="padding: 8px 16px; border-radius: 8px; border: none; cursor: pointer; background: #b47b47; color: white; font-weight: 600; text-decoration: none;"><i
                                        class="fas fa-eye"></i> Détails</a>
                                <?php if ($participation['statut'] === 'inscrit'): // Only show confirmer/annuler if status is 'inscrit' (pending) ?>
                                    <button class="btn-modifier" data-id="<?= htmlspecialchars($participation['id']) ?>"
                                        data-event-id="<?= htmlspecialchars($participation['id_evenement']) ?>"
                                        style="padding: 8px 16px; border-radius: 8px; border: none; cursor: pointer; background: #5e6d3b; color: white; font-weight: 600;"><i
                                            class="fas fa-check"></i> Confirmer</button>
                                    <button class="btn-annuler" data-id="<?= htmlspecialchars($participation['id']) ?>"
                                        data-event-id="<?= htmlspecialchars($participation['id_evenement']) ?>"
                                        data-user-id="<?= htmlspecialchars($userId) ?>"
                                        style="padding: 8px 16px; border-radius: 8px; border: none; cursor: pointer; background: #e74c3c; color: white; font-weight: 600;"><i
