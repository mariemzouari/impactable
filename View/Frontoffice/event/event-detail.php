        ...
        <div class="side-panel" id="sidePanel">
            ...
            <nav class="panel-nav">
                <div class="nav-section">
                    ...
                    <a href="<?= $nomDossierSite ?>/View/Frontoffice/index.php" class="nav-link"><i
                            class="fas fa-home"></i> <span>Accueil</span></a>
                    <a href="<?= $nomDossierSite ?>/View/Frontoffice/events-list.php" class="nav-link active"><i
                            class="fas fa-calendar-alt"></i> <span>Événements</span></a>
                </div>
            </nav>
        ...
        <section class="section">
            <div class="hero container" style="align-items:start;">
                ...
                <div class="hero-content">
                    ...
                    <div class="lead"
                        style="display: flex; flex-direction: column; gap: 10px; font-size: 0.95rem; color: #6b4b44; margin-bottom: 20px;">

                        <div>
                            <i class="fas fa-calendar-alt" style="color:#b47b47; width:20px;"></i>
                            Du <?php echo htmlspecialchars(date('d M Y à H:i', strtotime($event['date_debut']))); ?> au
                            <?php echo htmlspecialchars(date('d M Y à H:i', strtotime($event['date_fin']))); ?>
                        </div>
                        ...
                        <div style="display:flex; align-items:center;">
                            ...
                            <span
                                style="background:rgba(180,123,71,0.1); padding:2px 8px; border-radius:4px; font-weight:600;">
                                <?php echo $category; ?>
                            </span>
                        </div>
                    ...
                    <div
                        style="background:var(--card-bg); padding:20px; border-radius:var(--radius); margin-bottom:20px; text-align:left;">
                        <h2 style="margin-top:0;">Description</h2>
                        <p style="white-space:pre-line; line-height:1.7; color:var(--muted);">
                    ...
                    <?php if (!empty($event['location'])): ?>
                        <div
                            ...
                            <div id="map-container"
                                style="width: 100%; height: 300px; background-color: #e0e0e0; display: flex; align-items: center; justify-content: center; color: #555; border-radius: 8px; overflow: hidden;">
                                <iframe src="https://www.google.com/maps/embed?q=R6QP%2BVMQ%2C%20Tunis" width="100%"
                            ...
                            <p style="text-align: center; margin-top: 10px;">
                                <a href="https://www.google.com/maps/search/?api=1&query=<?= urlencode($event['location']) ?>"
                                    target="_blank" style="color: #5e6d3b; text-decoration: none;">
                    ...
                    <div class="hero-cta">
                        <?php if ($isParticipating): ?>
                            <p class="participation-status"><i class="fas fa-check-circle"></i> Vous participez à cet
                                événement !</p>
                            <button type="button" onclick="cancelParticipation(<?= $participationDetails['id'] ?>)"
                                class="btn primary" style="background: #e74c3c;"><i class="fas fa-times"></i> Annuler
                                participation</button>
                        <?php else: ?>
                            <button type="button" onclick="openParticipationModal()" class="btn primary"><i
                                    class="fas fa-heart"></i> Participer</button>
                        <?php endif; ?>

                        <?php if ($userId): ?>
                            <button id="favoriteButton" type="button"
                                class="btn ghost <?= $isFavorited ? 'favorited' : '' ?>"
                                data-event-id="<?= htmlspecialchars($eventId) ?>">
                                <i class="far fa-star"></i> <span
                                    class="button-text"><?= $isFavorited ? 'Retirer des favoris' : 'Ajouter aux favoris' ?></span>
                            </button>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div style="width: 100%; display:flex; justify-content:center;">
                    <img src="<?= htmlspecialchars($mainImageUrl) ?>" alt="<?= htmlspecialchars($event['titre']) ?>"
                        class="hero-img"
            ...
            <div id="participationModal" class="modal-backdrop hidden" aria-hidden="true">
                <div class="modal">
                    <div class="modal-header">
                        ...
                        <button class="modal-close" type="button" onclick="closeParticipationModal()"><i
                    ...
                    <div class="modal-body">
                        <form id="participationForm">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="num_tel" name="num_tel" class="form-input"
                                    placeholder="Votre numéro de téléphone" />
                            </div>
                            <div class="form-group">
                                <label for="num_identite">Numéro d'identité</label>
                                <input type="text" id="num_identite" name="num_identite" class="form-input"
                                    placeholder="Votre numéro d'identité" />
                            </div>

                            <div class="form-group">
                                <label for="nombre_accompagnants">Nombre d'accompagnants</label>
                                <input type="number" id="nombre_accompagnants" name="nombre_accompagnants"
                            ...
                            <div class="form-footer">
                                <button type="button" class="btn-modal secondary"
                                    onclick="closeParticipationModal()">Annuler</button>
