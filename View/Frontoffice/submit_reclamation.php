...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Use logged in user ID if available, otherwise default to 1 (or handle error)
        $utilisateurId = isset($_SESSION['user_id']) ? intval($_SESSION['user_id']) : (isset($_POST['utilisateurId']) ? intval($_POST['utilisateurId']) : 0);

        if ($utilisateurId <= 0) {
            echo json_encode(['success' => false, 'message' => 'Vous devez être connecté pour soumettre une réclamation.']);
            exit;
        }
        ...
        if (
            empty($sujet) || empty($description) || empty($categorie) || empty($priorite) ||
