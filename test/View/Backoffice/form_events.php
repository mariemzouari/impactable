...
$eventDateRaw = $event['date_event'] ?? '';
// datetime-local expects format YYYY-MM-DDTHH:MM
$eventDate = $eventDateRaw ? date('Y-m-d\TH:i', strtotime($eventDateRaw)) : '';
...
<form action="<?= htmlspecialchars($formAction) ?>" method="POST" class="event-form">

    <!-- TITRE -->
    <div class="form-group">
        <label for="titre">Titre de l'événement</label>
        <input type="text" id="titre" name="titre" class="form-input"
               placeholder="Ex : Nettoyage de plage" required value="<?= htmlspecialchars($eventTitle) ?>">
    </div>

    <!-- DATE ET HEURE -->
    <div class="form-group">
        <label for="date_event">Date & Heure</label>
        <input type="datetime-local" id="date_event" name="date_event" class="form-input" required value="<?= $eventDate ?>">
    </div>
    ...
    <div class="form-group">
        ...
        <input type="text" id="categorie" name="categorie" class="form-input"
               placeholder="Ex : Environnement, Sensibilisation…" required value="<?= htmlspecialchars($eventCategorie) ?>">
    </div>
    ...
    <div class="form-group">
        ...
        <textarea id="description" name="description" rows="4" class="form-input"
                  placeholder="Décrivez brièvement l'événement…" required><?= htmlspecialchars($eventDescription) ?></textarea>
    </div>
