...
class PublicParticipationController {
    ...
    private function participateWithDetails($data) {
        ...
        $criteria = [
            ...
            'statut' => ['inscrit', 'confirmé'] // Check for these statuses
        ];

        if ($isGuest) {
            // For guests, check by email and ensure no id_utilisateur
            $criteria['email'] = $data['email'];
            $criteria['id_utilisateur'] = null; // Explicitly ensure we're looking for guest participation

            // If the guest provides an email, try to find an existing user
            if (!empty($data['email'])) {
                $stmt = $this->db->prepare("SELECT id FROM utilisateur WHERE email = :email LIMIT 1");
                ...
                if ($existingUser) {
                    ...
                    $isGuest = false; // No longer a guest for this participation
                    $criteria['id_utilisateur'] = $data['user_id']; // Update criteria for existing user
                } else {
                    // Create a minimal user for this guest if they don't exist
                    $displayName = trim(($data['prenom'] ? $data['prenom'] . ' ' : '') . $data['nom']) ?: 'Guest';
                    $insertUserStmt = $this->db->prepare("INSERT INTO utilisateur (nom, prenom, email, password, role, created_at) VALUES (:nom, :prenom, :email, :password, :role, NOW())");
                    ...
                    $isGuest = false; // No longer a guest for this participation as a user record is created
                    $criteria['id_utilisateur'] = $data['user_id']; // Update criteria
                }
            }
        } else {
        ...
        $participationData = [
            ...
            'statut' => 'inscrit' // Explicitly set status to 'inscrit' (en attente)
        ];
