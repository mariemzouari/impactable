...
<body>
    <div class="admin-container">
        ...
        <div class="admin-main">
            ...
            <div class="notification notification-warning" style="background: #FFF3CD; border: 1px solid #B47B47; color: #856404; padding: 15px 20px; border-radius: 10px; margin-bottom: 20px; display: flex; align-items: center; gap: 10px;">
                <i class="fas fa-info-circle"></i>
                <span>Veuillez sélectionner une réclamation dans le tableau ci-dessous pour voir ses détails.</span>
                <button onclick="this.parentElement.style.display='none'" style="margin-left: auto; background: none; border: none; cursor: pointer; font-size: 1.2em;">&times;</button>
            </div>
            <?php endif; ?>
            ...
            <div class="admin-content">
                ...
                <?php if (isset($_GET['success']) && $_GET['success'] == 1): ?>
                    <div style="background: rgba(46, 204, 113, 0.1); color: #27ae60; padding: 15px 20px; border-radius: 10px; margin-bottom: 24px; font-weight: 600;">
                                ...
                                <?php if (empty($reclamations)): ?>
                                    <tr>
                                        <td colspan="8" style="text-align: center; padding: 60px 20px; color: var(--muted);">
                                            <i class="fas fa-inbox" style="font-size: 3em; margin-bottom: 16px; display: block; opacity: 0.5;"></i>
                                ...
                                <?php else: ?>
                                    <?php foreach ($reclamations as $rec): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <div style="display: flex; align-items: center; gap: 10px;">
                                                    <div class="user-avatar">
                                                        <?= isset($rec['prenom']) ? strtoupper(substr($rec['prenom'], 0, 1)) : 'U' ?>
                                                    </div>
                                                    <div>
                                            ...
                                            <td><?= htmlspecialchars(substr($rec['sujet'], 0, 30)) ?><?= strlen($rec['sujet']) > 30 ? '...' : '' ?></td>
                                            <td><?= htmlspecialchars($rec['categorie']) ?></td>
                                            <td>
                                                <?php
                                                $prioClass = 'pending';
                                                if ($rec['priorite'] == 'Urgente') $prioClass = 'rejected';
                                                elseif ($rec['priorite'] == 'Faible') $prioClass = 'active';
                                                ?>
                                                <span class="status <?= $prioClass ?>"><?= htmlspecialchars($rec['priorite']) ?></span>
                                            </td>
                                            <td>
                                                <?php
                                                $statClass = 'pending';
                                                if ($rec['statut'] == 'Résolue') $statClass = 'active';
                                                elseif ($rec['statut'] == 'Fermée') $statClass = 'inactive';
                                                elseif ($rec['statut'] == 'En cours') $statClass = 'pending';
                                                ?>
                                                <span class="status <?= $statClass ?>"><?= htmlspecialchars($rec['statut']) ?></span>
                                            </td>
                                            <td>
                                                <div class="table-actions">
                                                    <a href="gestion_reclamation/showReclamation.php?id=<?= $rec['id'] ?>" class="btn small ghost" title="Voir">
                                                        <i class="fas fa-eye"></i>
                                                    </a>
                                                    <a href="gestion_reclamation/updateReclamation.php?id=<?= $rec['id'] ?>" class="btn small ghost" title="Modifier">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <a href="?delete=<?= $rec['id'] ?>" class="btn small danger" title="Supprimer" 
